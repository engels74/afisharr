...
| 2.1 | The fake, corrected against a reference client | `L` |
...
## Phase 2.1 — The fake, corrected against a reference client

**Size:** `L`.

**Exit invariants:** Phase 2's, re-checked — `I-ID-5`, plus the fidelity contract. Plus this phase's own
criterion: every call in the surface is read correctly by an independent Plex client that was not
written here, and the release-lane contract test of Task 2.3 passes in both directions.

**Not here:** anything that consumes Plex data, and any new protocol call. This phase changes what the
fake answers and what the tests read. It adds one client call only where the audit found a call the
product needs and does not have.

**Why this phase exists.** Phase 2 built the fake and its fidelity contract, and both are sound in the
part they were written for: the misbehaviours reproduce, the seed replays, and the client parses. What
neither covers is whether the *shapes* are the ones a Plex Media Server actually sends. An audit
against `python-plexapi` 4.18.2 — the most complete reverse-engineered Plex client, and the reference
this phase treats as the source of truth for server behaviour — found sixteen places where they are
not, four of them load-bearing for Phases 6 and 7. Every one of them makes a test pass that a real
server would fail.

**Why it could not have been caught earlier.** The fake and the client were written together, against
each other, from the same reading of the protocol. `json.rs` says so in its own header: the shapes are
hand-written rather than re-serialised from the client's types, so the two would not agree by
construction. They agree by *authorship* instead, which is the same failure one step back. The
release-lane contract test (Task 2.3) is the designed answer to this and its **Done when** box is not
ticked — no real server has ever answered it. Until one does, `python-plexapi` is the only reader in
reach that was written against real servers by people who were not in this room.

**Why a reference client rather than more fixtures.** A fixture this repository writes is another
claim by the same author. A second client is a second reader: it fails on a wrong attribute name
whether or not anybody here suspected that name. It also reads XML, which is what a real Plex answers
by default, and the fake has never emitted a byte of it.

**The four that block Phase 6 and Phase 7:**

| # | What the fake does | What a real server does | Evidence |
| --- | --- | --- | --- |
| 1 | Names a manageable hub with `hubIdentifier`, and marks Plex's own rows by omitting `ratingKey` | Names it `identifier`, and marks a removable row with `deletable` | `plexapi/library.py:3037`, `:3035`; the class that reads `hubIdentifier` is `Hub`, a different endpoint (`:2226`, `:725`) |
| 2 | Lists a never-promoted collection in `/hubs/sections/{key}/manage` | Answers no row for it; the client synthesises one and promotes it by `POST` with `metadataItemId` | `plexapi/collection.py:207-215`, `plexapi/library.py:3114-3120` |
| 3 | Routes every non-label edit to a collection, so an item's sort title cannot be written at all | Edits whatever `id` names, at the libtype `type` names | `plexapi/library.py:1743-1755`, `plexapi/base.py:722-725` |
| 4 | Reports `collectionSort` as `2` always, and discards an edit that sets it | Defaults it to `0`, and writes what it is sent | `plexapi/collection.py:73` |

Rows 1 and 2 are the ordering space, which is all of Phase 7. Row 3 is the sort-title round trip, which
is Task 7.7. Row 4 is the precondition for every item move meaning anything, which is Task 6.1.

### Task 2.1.1 The XML surface, and a reference client that reads it
- **Build:** content negotiation in the fake, and a cross-check lane that drives it with
  `python-plexapi` rather than with this repository's own parsers.
- **Where:** `backend/crates/plex/src/fake/`, a new `xml` module beside `json`; the cross-check under
  `backend/crates/plex/tests/reference/`, wired into the nightly and release lanes from Task 0.1 and
  **not** into the merge lane, which stays Rust-only and needs no Python.
...
### Task 2.1.2 The manage-hub surface
- **Build:** the ordering space as a real server presents it, and the two endpoints the fake does not
  serve at all.
...
### Task 2.1.3 The edit endpoint
- **Build:** `PUT /library/sections/{key}/all` as one endpoint over every libtype, with the argument
...
### Task 2.1.4 The container and the collection envelope
- **Build:** the container-level and collection-level attributes a client addresses the rest of the
  surface by.
...
### Task 2.1.5 The request arguments the fake ignores
- **Build:** a fake that answers the question it was asked.
...
### Task 2.1.7 The contract test, both directions and over writes
...
### Task 2.1.8 A world wide enough to fail against
